...
export default function DashboardPage() {
  ...
  const userChartData = stats?.newUsersByMonth?.map(item => ({
    name: `T${item._id}`,
    users: item.count
  })) || [];
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-muted-foreground">Chào mừng trở lại, đây là những gì đang diễn ra với cửa hàng của bạn.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-card/50 backdrop-blur-sm border-primary/10">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(stats?.totalRevenue || 0)}</div>
            <p className="flex items-center text-xs text-emerald-500 mt-1">
              <ArrowUpRight className="mr-1 h-3 w-3" /> +20.1% so với tháng trước
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {stats?.lowStockProducts?.slice(0, 5).map((product) => (
                <div key={product._id} className="flex items-center gap-4 rounded-lg border p-3 hover:bg-muted/50 transition-colors">
                  <img src={product.image} className="h-10 w-10 object-contain rounded bg-white p-1 border" />
                  <div className="flex-1">
                    <p className="text-sm font-semibold">{product.name}</p>
                    <p className="text-xs text-muted-foreground">Còn lại: <span className="text-destructive font-bold">{product.countInStock}</span></p>
                  </div>
              ...
              {stats?.lowStockProducts?.length === 0 && <p className="text-muted-foreground italic text-center py-4">Tất cả sản phẩm đều đủ hàng.</p>}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="flex flex-col gap-6 justify-center h-[300px]">
             <div className="space-y-2">
                ...
                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                   <div className="h-full bg-primary rounded-full transition-all duration-1000" style={{ width: `${Math.min(100, stats?.conversionRate * 100)}%` }} />
                </div>
             </div>
             <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                   <span className="text-muted-foreground font-medium">Sự hài lòng khách hàng</span>
             ...
             <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                   <span className="text-muted-foreground font-medium">Thời gian giao hàng trung bình</span>
                   <span className="font-bold">1.2 ngày</span>
                </div>
                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                   <div className="h-full bg-blue-500 rounded-full transition-all duration-1000" style={{ width: `85%` }} />
                </div>
             </div>
          </CardContent>
        </Card>
